...
# Also adapted from https://github.com/andy-pi/weather-monitor/blob/master/air_quality.py
...
class sensor:
   ...
   def __init__(self, pi, gpio):
      ...
      pi.set_mode(gpio, onionGpio.INPUT)

      self._cb = pi.callback(gpio, onionGpio.EITHER_EDGE, self._cbf)
   ...
   def _cbf(self, gpio, level, tick):
      ...
      if self._start_tick is not None:
         ...
         ticks = onioGgpio.tickDiff(self._last_tick, tick)
...
#   import pigpio
   ...
   while True:
      pi = onionGpio.pi() # Connect to Pi.
   
      s = sensor(pi, 18) # set the GPIO pin number
